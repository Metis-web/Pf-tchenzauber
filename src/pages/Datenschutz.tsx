...
export default function Datenschutz() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'settings', 'legal')).then((docSnap) => {
      if (docSnap.exists() && docSnap.data().datenschutz) {
        setContent(docSnap.data().datenschutz);
      } else {
        setContent("Keine Datenschutzerklärung hinterlegt.");
      }
    }).catch((err) => {
      console.error(err);
  ...
  return (
    <div className="py-16 sm:py-24 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 bg-white min-h-[60vh]">
      <h1 className="font-display text-4xl font-black text-stone-900 mb-8">Datenschutzerklärung</h1>
      
      <div className="prose prose-stone max-w-none whitespace-pre-wrap leading-relaxed">
        {content !== "Lädt..." && content !== "Fehler beim Laden." && content !== "Keine Datenschutzerklärung hinterlegt." ? (
            content
        ) : (
